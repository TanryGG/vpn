...
function requireAdmin3ForRoleChanges(req,res,next){
  if (adminPermissions.getAdminLevel(req) >= 3) return next();

  if (res.render) {
    return res.status(403).render('error', {
      title: 'Доступ запрещён',
      message: 'Менять роли пользователей может только администратор 3 уровня.'
    });
  }

  return res.status(403).send('Менять роли пользователей может только администратор 3 уровня.');
}


function blockRoleFieldsForNonAdmin3(req,res,next){
  const body = req.body || {};
  const hasRoleField =
    Object.prototype.hasOwnProperty.call(body, 'role') ||
    Object.prototype.hasOwnProperty.call(body, 'admin_role') ||
    Object.prototype.hasOwnProperty.call(body, 'adminRole') ||
    Object.prototype.hasOwnProperty.call(body, 'admin_level') ||
    Object.prototype.hasOwnProperty.call(body, 'adminLevel') ||
    Object.prototype.hasOwnProperty.call(body, 'role_level') ||
    Object.prototype.hasOwnProperty.call(body, 'roleLevel') ||
    Object.prototype.hasOwnProperty.call(body, 'level');

  if (!hasRoleField) return next();
  if (adminPermissions.getAdminLevel(req) >= 3) return next();

  return res.status(403).render ? res.status(403).render('error', {
    title: 'Доступ запрещён',
    message: 'Менять роли пользователей может только администратор 3 уровня.'
  }) : res.status(403).send('Менять роли пользователей может только администратор 3 уровня.');
}
...
router.use((req,res,next)=>{
  res.locals.adminLevel = adminPermissions.getAdminLevel(req);
  res.locals.adminRoleKey = adminPermissions.roleKeyByLevel(res.locals.adminLevel);
  res.locals.adminMenu = adminPermissions.allowedMenu(req);
  next();
});
...
function getTelegramProxyAdminStats(){
  const out = {
    configured: false,
    host: '',
    port: '',
  ...
  try {
    const ss = execSync("ss -lntp 2>/dev/null | grep -E ':8445|:8443|:443' || true", {encoding:'utf8'});
    const lines = ss.split('\n').filter(Boolean);
    out.listenLine = lines.join('\n');
    out.listen = lines.some(x => out.port && x.includes(':' + out.port));
  } catch(e) {}

  try {
    const logs = execSync("journalctl -u danted --since '24 hours ago' --no-pager 2>/dev/null | tail -120 || true", {encoding:'utf8'});
    const lines = logs.split('\n').filter(Boolean);
    out.connections24 = (logs.match(/pass\(1\): tcp\/accept/g) || []).length;
    out.authFails24 = (logs.match(/system password authentication failed/g) || []).length;
    out.lastLines = lines.slice(-12);
  } catch(e) {}
...
router.get('/', async (req,res)=>{
  const stats = {
    ...
    telegramProxy: getTelegramProxyAdminStats(),
  };
...
router.post('/telegram-proxy/autosetup', adminPermissions.requireFeature('telegram_proxy'), async (req,res)=>{const {execFile}=require('child_process');const script=require('path').join(process.cwd(),'scripts','ag-proxy-autosetup.sh');execFile('bash',[script],{cwd:process.cwd(),timeout:240000,env:Object.assign({},process.env,{APP_DIR:process.cwd(),TG_PROXY_HOST:String((req.body&&req.body.host)||'astragate.su').trim(),TG_PROXY_PORT:String((req.body&&req.body.port)||'8445').trim()})},(err,stdout,stderr)=>{console.log('[telegram-proxy-autosetup]',stdout,stderr);if(req.flash){err?req.flash('error','Автонастройка Telegram Proxy не выполнена'):req.flash('success','Telegram Proxy настроен')}res.redirect('/admin/settings#telegram-proxy')});});


router.get('/telegram-proxy', adminPermissions.requireFeature('telegram_proxy'), async (req,res)=>{
  res.render('admin/telegram_proxy', {
    title: 'Telegram Proxy',
    tp: getTelegramProxyAdminStats(),
    csrfToken: req.csrfToken ? req.csrfToken() : ''
  });
});

router.post('/telegram-proxy/autosetup', adminPermissions.requireFeature('telegram_proxy'), async (req,res)=>{
  const host = String((req.body && req.body.host) || 'astragate.su').replace(/[^a-zA-Z0-9.\-]/g,'') || 'astragate.su';
  const port = String((req.body && req.body.port) || '8445').replace(/[^0-9]/g,'') || '8445';
  const password = String((req.body && req.body.password) || 'AstraGate8445').replace(/[^a-zA-Z0-9_\-]/g,'') || 'AstraGate8445';

  const script = `
TG_PROXY_HOST="${host}"
TG_PROXY_PORT="${port}"
TG_PROXY_USER="tgproxy"
TG_PROXY_PASS="${password}"
IFACE="$(ip route get 1.1.1.1 | awk '{for(i=1;i<=NF;i++) if($i=="dev"){print $(i+1); exit}}')"
[ -n "$IFACE" ] || IFACE="eth0"
id "$TG_PROXY_USER" >/dev/null 2>&1 || useradd -r -s /usr/sbin/nologin "$TG_PROXY_USER"
usermod -U "$TG_PROXY_USER" 2>/dev/null || true
echo "$TG_PROXY_USER:$TG_PROXY_PASS" | chpasswd
cat > /etc/danted.conf <<EOF
logoutput: syslog

internal: 0.0.0.0 port = $TG_PROXY_PORT
external: $IFACE
...
{"type":"mtproto","host":"$TG_PROXY_HOST","port":"$TG_PROXY_PORT","username":"$TG_PROXY_USER","password":"$TG_PROXY_PASS","url":"tg://proxy?server=astragate.su&port=8445&secret=<%= tp.secret || telegramProxyConfig.secret || "" %>","price_month_rub":50}
EOF
...
`;
  ...
  res.redirect('/admin/telegram-proxy');
});

router.get('/permissions', adminPermissions.requireLevel3, async (req,res)=>{
  res.render('admin/permissions', {
    title: 'Разрешения',
    features: adminPermissions.FEATURES,
    permissions: adminPermissions.loadPermissions(),
    saved: req.query.saved === '1',
    csrfToken: req.csrfToken ? req.csrfToken() : ''
  });
});

router.post('/permissions', adminPermissions.requireLevel3, async (req,res)=>{
  const body = req.body || {};
  const data = {};
  for (const role of ['moderator','admin1','admin2','admin3']) {
    data[role] = {};
    for (const key of Object.keys(adminPermissions.FEATURES)) {
      data[role][key] = body[role + '__' + key] === '1';
    }
  }
  adminPermissions.savePermissions(data);
